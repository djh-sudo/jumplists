...
public:
	...
	void SetLastAccessTime(FILETIME fileTime) {
		this->m_lastRecordTime = fileTime;
	}

	void SetCreateTime(FILETIME fileTime) {
		this->m_createTime = fileTime;
		return;
	}

	void SetModifyTime(FILETIME fileTime) {
		this->m_modifyTime = fileTime;
		return;
	}
	...
	FILETIME GetLastRecordTime() const {
	...
	DL_ENTRY() {
		memset(&m_lastRecordTime, 0, sizeof(m_lastRecordTime));
...
public:
	...
	bool Init(const void * buffer, int dwBuffer) {
		if (buffer == NULL || dwBuffer <= 0) {
			return false;
		}
		else {
			m_memoryBuffer = (PBYTE)buffer;
			m_dwMemoryBuffer = dwBuffer;
		}
		SATChain.clear();
		SSATChain.clear();
		memcpy(&m_oleHeader, m_memoryBuffer, sizeof(m_oleHeader));
	...
	bool GetDirs() {
		...
		for (DWORD i = 0; i < countSector && it != end; ++i, it++) {
			offset = GetBlock(*it);
			if (offset >= m_dwMemoryBuffer) {
				return false;
			}
			for (int k = 0; k < 4; ++k) {
				memcpy(m_dirEntrys + i * 4 + k, m_memoryBuffer + offset, DIR_SIZE);
	...
	bool AquireSATChain() {
		...
		for (DWORD i = 0; i < m_oleHeader.countSAT; ++i) {
			...
			memcpy(sector, m_memoryBuffer + offset, SECTOR_SIZE);
				...
				if (sector[j] == ID_SAT) {
					...
					do {
						if (index >= prefix) {
							...
							if (SATChain.find(index) == SATChain.end()) {
								return false;
							}
				...
				if (sector[k] == ID_SAT) {
					...
					do {
						if (index >= prefix) {
							...
							if (SSATChain.find(index) == SSATChain.end()) {
								return false;
							}
							// merge
	...
	bool GetDestListFromSAT(std::vector<DL_ENTRY>& res) {
		...
		bool endFlag = false;
		DWORD i = 0;
		for (i = 0; i < 4 && it != end; ++i) {
			offset = GetBlock(*it++);
			if (offset >= m_dwMemoryBuffer) {
				return false;
			}
			memcpy(m_LoopBuffer[i].buffer, m_memoryBuffer + offset, SECTOR_SIZE);
		}
		...
		bool succeed = true;
		for (i = 0; i < dwEntrys && succeed;) {
			memset(content, 0, SECTOR_SIZE << 1);
			UpdateMemory(content, loopId, dwRead);
			// Analysis DestList
			dwOldRead = AnalyseDestList(content, i, res);
			if (dwOldRead == 0) {
				// slow path
				if (i < dwEntrys) {
					...
					dwOldRead = AnalyseDestList(largeMemory, i, res, SECTOR_SIZE << 1);
				}
				if (dwOldRead == 0) {
					break;
				}
			...
			dwRead += dwOldRead;
			while (dwRead > SECTOR_SIZE && succeed) {
				if (it != end) {
					succeed = UpdateSector(*it++, loopId, dwRead, endFlag);
					continue;
				}
				if (it == end) {
					succeed = UpdateSector(0, loopId, dwRead, true);
				}
			}
		}
		if (i < dwEntrys) {
	...
	DWORD GetSSAT(PBYTE buffer, DWORD start) {
		...
		for (int k = 0; ssatIt != ssatEnd && it != end; k++) {
			...
			while (ssatIt != ssatEnd && GetMiniBlock(*ssatIt) < threshHold) {
				...
				if (offset + index > m_dwMemoryBuffer) {
					return 0;
				}
				memcpy(buffer + dwRead, m_memoryBuffer + offset + index, SHORT_SECTOR_SIZE);
				ssatIt++;
	...
	bool GetDestListFromSSAT(std::vector<DL_ENTRY>&res) {
		...
		for (i = 0; i < dwEntrys;) {
			// Analysis DestList
			dwOldRead = AnalyseDestList(content + dwRead, i, res);
...
private:
	...
	inline bool CheckValid() {
		return !(m_memoryBuffer == NULL || m_oleHeader.SIDs == NULL);
	}
	...
	DWORD AnalyseDestList(const BYTE* content, DWORD& counter, std::vector<DL_ENTRY>&res, DWORD baseLen = SECTOR_SIZE) {
		...
		do {
			...
			if (((entry->szPath << 1) + WIN_10_ENTRY + 4 + dwRead) < (baseLen << 1)) {
				obj.Init();
				obj.SetLastAccessTime(entry->lastAccessTime);
				...
				if (m_lnk.HasTime()) {
					obj.SetCreateTime(m_lnk.GetCreateTime());
					obj.SetModifyTime(m_lnk.GetWriteTime());
				}
				res.push_back(obj);
	...
	bool UpdateSector(const DWORD off, DWORD& loopId, DWORD& dwRead, bool flag = false) {
		// step 1 Load an new sector
		if (!flag) {
			...
			memcpy(m_LoopBuffer[loopId].buffer, m_memoryBuffer + offset, SECTOR_SIZE);
		}
		else {
			memset(m_LoopBuffer[loopId].buffer, 0, SECTOR_SIZE);
		}
	...
	void UpdateMemory(BYTE* content, const DWORD loopId, const int dwRead) {
...
public:
	...
	static void GetContent(const void* memoryBuffer, DWORD dwBuffer, std::vector<DL_ENTRY>& res) {
		OLE_OBJECT ole;
		bool flag = false;
		do {
			flag = ole.Init(memoryBuffer, dwBuffer);
			if (flag == false) {
				break;
			}

			flag = ole.AquireSATChain();
			if (flag == false) {
				break;
			}
			...
			flag = ole.AquireSSATChain();
			if (flag == false) {
				break;
			}

			if (ole.GetdwDestList() < 4096) {
				flag = ole.GetDestListFromSSAT(res);
			}
			else {
				flag = ole.GetDestListFromSAT(res);
			}
